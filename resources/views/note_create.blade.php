...
<div class="container col-md-8">
    ...
    <form action="{{ route('note_store') }}" method="post" novalidate>
        ...
        <div class="mb-3">
            <label for="swing" class="form-label"><strong>素振り回数 </strong><span class="badge bg-danger">必須</label>
            <input type="number" class="form-control @error('swing') is-invalid @enderror" id="swing" name="swing" value="{{ old('swing') }}" placeholder="50" required>
            @error('swing')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" value="{{ old('weight') }}" placeholder="70.5" step="0.1" required>
            @error('weight')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            <label for="memo" class="form-label"><strong>今日の振り返り（気づきや学び、感想など自由に記述）</strong><span class="badge bg-danger">必須</label>
            <textarea class="form-control @error('memo') is-invalid @enderror" id="memo" name="memo" value="{{ old('memo') }}" rows="3" required></textarea>
            @error('memo')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
